...
export default function ExpenseCategoriesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Gider kategorisi silindi');
    },
  });

  const createMutation = useMutation({
    mutationFn: (data: { name: string }) => expenseCategoryApi.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['expense-categories'] });
      toast.success('Gider kategorisi eklendi');
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['expense-categories'] });
      toast.success('Gider kategorisi güncellendi');
      setEditingId(null);
    },
  });
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-gray-800 mb-6">Gider Kategorileri</h1>

      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">Yeni Gider Kategorisi Ekle</h2>
